...
export default class ReactTable extends Methods(Lifecycle(Component)) {
  ...
  constructor (props) {
    ...
    if (props.useVirtualization && (props.virtualizationHeight === undefined || props.virtualizationRowHeight === undefined)) {
      throw new Error('When useVirtualization is true, virtualizationHeight and virtualizationRowHeight needs to be set.')
    }
  }
  ...
  render () {
    ...
    const makePageRow = (row, i, path = [], style) => {
      const rowInfo = {
      ...
      const trPropsTemp = getTrProps(finalState, rowInfo, undefined, this)
      if (style) {
        if (!trPropsTemp.style) {
          trPropsTemp.style = {}
        }

        Object.entries(style).forEach(([k, v]) => {
          trPropsTemp.style[k] = v
        })
      }
          ...
          >
            ...
            {useVirtualization ?
              <FixedSizeList
                ...
                itemSize={virtualizationRowHeight}
                width="100%"
                style={{ overflowX: 'hidden' }}
              >
                {({ index, style }) => makePageRow(pageRows[index], index, [], style)}
              </FixedSizeList>
